...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCraveAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCraveAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCraveAmount = strAnonymizeCraveAmount.remove(strAnonymizeCraveAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCraveAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCraveAmount*COIN) nMaxToAnonymize = nAnonymizeCraveAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCraveAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCraveAmount));
        strAnonymizeCraveAmount = strAnonymizeCraveAmount.remove(strAnonymizeCraveAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCraveAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
