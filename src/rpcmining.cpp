...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (int64_t)GetProofOfStakeReward(pindexBest->pprev, 0, 0, pindexBest->nHeight);
}
...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest->pprev, nCoinAge, 0, pindexBest->nHeight);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (int64_t)GetProofOfStakeReward(pindexBest->pprev, 0, 0, pindexBest->nHeight)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Crowd is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Crowd is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Crowd is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Crowd is downloading blocks...");
